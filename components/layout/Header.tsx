    ...
    <header className="sticky top-0 z-20 bg-white/90 border-b border-slate-200 backdrop-blur">
      <div className="flex items-center gap-3 px-4 sm:px-6 h-16">
        <button onClick={onOpenNav} aria-label="Open navigation"
          className="lg:hidden p-2 -ml-2 rounded-lg text-slate-600 hover:bg-slate-100 focus:outline-none focus-visible:ring-2 focus-visible:ring-amber-400">
          <Menu size={20} />
        </button>
        ...
        <button onClick={onToggleDemo} title="Demo controls"
          ...
          className={`p-2 rounded-lg transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-amber-400 ${showDemo ? "bg-amber-50 text-amber-600" : "text-slate-500 hover:bg-slate-100"}`}>
          <SlidersHorizontal size={18} />
        </button>
        <Link href="/alerts" aria-label={unread > 0 ? `Alerts, ${unread} unread` : "Alerts"} className="relative p-2 rounded-lg text-slate-500 hover:bg-slate-100 focus:outline-none focus-visible:ring-2 focus-visible:ring-amber-400">
          <Bell size={18} />
        ...
        <Link href="/household" aria-label="Household" title="Household"
          className="p-2 rounded-lg text-slate-500 hover:bg-slate-100 focus:outline-none focus-visible:ring-2 focus-visible:ring-amber-400">
          <Users size={18} />
        </Link>
