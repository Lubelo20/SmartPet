...
export function StatusCard({ label, value, unit, caption, icon: Icon, tone = "neutral", children, onClick }: StatusCardProps) {
  ...
  return (
    <Card className={`p-5 ${onClick ? "cursor-pointer hover:border-line transition-colors" : ""}`} onClick={onClick}>
      <div className="flex items-start justify-between gap-3">
        <Label>{label}</Label>
        <span className={`w-8 h-8 rounded-xl flex items-center justify-center ${t.bg} ${t.text}`}><Icon size={16} /></span>
      </div>
      <div className="mt-3 flex items-baseline gap-1.5">
        <span className="text-3xl font-bold tracking-tight text-ink font-mono">{value}</span>
        {unit && <span className="text-sm font-medium text-muted">{unit}</span>}
      </div>
      {caption && <p className="text-sm text-muted mt-1">{caption}</p>}
